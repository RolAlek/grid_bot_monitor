...
class AssessPositioning:
    ...
    def assess(self, snapshot: FundingOiSnapshot) -> GateResult:
        ...
        if abs(funding) > self._settings.funding_annualized_fail_pct:
            ...
            reasons.append(
                f"Funding {funding:.1f}% annualized exceeds ±{self._settings.funding_annualized_fail_pct}% fail threshold"
            )
        ...
        if oi_change is None:
            ...
            crowded_long = (
                funding > self._settings.funding_annualized_caution_pct
                and oi_change > 0
            )
            crowded_short = (
                funding < -self._settings.funding_annualized_caution_pct
            ...
            if (
                crowded_long or crowded_short
            ) and oi_change > self._settings.oi_7d_change_fail_pct:
                statuses.append(GateStatus.FAIL)
                direction = "long" if crowded_long else "short"
                reasons.append(
                    f"Crowded {direction}: OI +{oi_change:.1f}% in 7d with extreme funding {funding:.1f}%"
                )
